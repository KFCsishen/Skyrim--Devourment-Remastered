...
Event OnEffectStart(Actor akTarget, Actor akCaster)
    ...
    if((thepred == Game.GetPlayer()&&(Thepred.getlevel() < theprey.getlevel()))||(thePred.getItemCount(Fullness)>0&&thePred == Game.GetPlayer()&&multiprey.getvalue() == 0) || (thePred.getItemCount(Fullness)>0&&thePred != Game.GetPlayer()&&multipreynpc.getvalue() == 0) )
        if(thepred == Game.GetPlayer()&&Thepred.getlevel() < theprey.getlevel())
            
            Debug.Notification("They are too strong for me to swallow!")
        elseif(Thepred == Game.GetPlayer())
